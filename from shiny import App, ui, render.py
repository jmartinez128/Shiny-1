...
def server(input, output, session):
    ...
    def gender_spending_comparison():
        filtered_df = apply_filters(df)
        gender_df = filtered_df.groupby("Gender")["Purchase_Amount_USD"].sum().reset_index()
    ...
    def category_spending_comparison():
        filtered_df = apply_filters(df)
        category_df = filtered_df.groupby("Category")["Purchase_Amount_USD"].sum().reset_index()
    ...
    def seasonal_category_heatmap():
        filtered_df = apply_filters(df)
        seasonal_category_df = filtered_df.groupby(["Season", "Category"])["Purchase_Amount_USD"].sum().unstack()
    ...
    def seasonal_spending_trends():
        filtered_df = apply_filters(df)
        seasonal_df = filtered_df.groupby("Season")["Purchase_Amount_USD"].sum().reset_index()
    ...
    def payment_method_comparison():
        ...
        payment_df = filtered_df.groupby("Payment_Method")["Purchase_Amount_USD"].sum().reset_index()
    ...
    def discount_promo_impact():
        filtered_df = apply_filters(df)
        promo_df = filtered_df.groupby("Discount_Applied")["Purchase_Amount_USD"].sum().reset_index()
